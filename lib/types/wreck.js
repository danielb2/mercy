...
module.exports = internals.Wreck = class extends Flow {
    ...
    _tasks () {
        ...
        const wreck = (...args) => {
            ...
            const options = Hoek.applyToDefaults(this._defaults.options, input.value.options);

            const base = new Uri(this._defaults.options.baseUrl || '').toString();
            const post = new Uri (input.value.uri || this._defaults.uri || '').toString();
    ...
    defaults (...args) {
        ...
        return internals.defaults.call(this, this._method, ...args);
    }
