...
module.exports = internals.Flow = class extends Any {
    ...
    constructor (tasks) {
        ...
        this._children = [];
        this._final = null;
        this._settings = Hoek.clone(internals.defaults);
    ...
    _execute (...args) {
        ...
        if (this._settings.timeout) {
            ...
            const _timeout = (data, next) => {
                ...
                const duration = this._settings.timeout;
        ...
        for (let i = 0; i < this._children.length; ++i) {
            ...
            // Mercy.series() (aka no dependencies listed) (`Rest ()` & `Array []` notation)
    ...
    series () {
        ...
        for (let i = 0; i < this._children.length; ++i) {
            const child = this._children[i];
            Hoek.assert(!child.depends.length, 'Cannot convert auto to series');
        }
    ...
    parallel () {
        ...
        for (let i = 0; i < this._children.length; ++i) {
            const child = this._children[i];
            Hoek.assert(!child.depends.length, 'Cannot convert auto to parallel');
        }
    ...
    auto () {
        ...
        for (let i = 0; i < this._children.length; ++i) {
            const child = this._children[i];
            Hoek.assert(!child.depends.length, 'Cannot convert to auto');
        }
    ...
    retry ({ times, interval }) {
        ...
        this._settings.retry = { times, interval };;
    ...
    required () {
        ...
        this._settings.optional = false;
        return this;
    }

    optional () {

        this._settings.optional = true;
        return this;
    }

    timeout (duration) {

        this._settings.timeout = duration || 0;
        return this;
    }
