...
module.exports = internals.Flow = class extends Any {
    ...
    _execute (...args) {
        ...
        if (this._settings.mock) {
            const _mock = (data, next) => {
                ...
                const { mode, local, label, dir } = this._settings.mock;
                ...
                const options = {
                    afterRecord: (scopes) => {
                        ...
                        // console.log({ mode, local, label, dir });
                        // console.log()
                ...
                nockback(`${label}.json`, options, function (done) {

                    // console.log('nockback')
                    // console.log(nockback)
                    // console.log({ mode, local, label, dir });
                    // console.log({ this: this.scopes });
                    // console.log()
                    ...
                    return next(null, { mode, local, label, dir, done });
                });
            };
    ...
    mock (options) {
        ...
        // `flow().mock(options)`
        // - `dir`: _string().requried()_; Directory to record/use fixtures.
        // - `label`: _string().requried()_; Used for fixture filenames and contextualization.
