...
module.exports = internals.Flow = class extends Any {
    ...
    _execute(...args) {
        ...
        if (this._settings.mock) {
            const _mock = (data, next) => {
                ...
                const { mode, local, fixture } = this._settings.mock;
    ...
    mock(options) {
        ...
        const schema = Joi.object().required().keys({
            ...
            loader: Joi.object().keys({ after: Joi.func().arity(1), before: Joi.func().arity(1) }),
            recorder: Joi.object().keys({ after: Joi.func().arity(1) }),
